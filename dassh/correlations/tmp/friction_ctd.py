...
def _calc_sc_ff_const(asm, wd, ws):
    ...
    if asm.wire_diameter == 0.0:
        ...
        for r in ['laminar', 'turbulent']:
            ...
            Cf[r][2] = Cfb[r][2]
            Cf[r][2] *= (1 + (ws[r] * (asm.params['wproj'][2]
...
def calculate_subchannel_friction_factors(asm):
    """Calculate the Cheng-Todreas Detailed friction factors for
    each subchannel
    ...
    -------
    numpy.ndarray
        Friction factors in the interior, edge, corner subchannels

    """
    try:
        Re_bl, Re_bt = asm.corr_constants['ff']['Re_bnds']
    except (KeyError, AttributeError):
        Re_bl, Re_bt = calculate_Re_bounds(asm)

    try:
        cfsc = asm.corr_constants['ff']['Cf_sc']
    except (KeyError, AttributeError):
        cfsc = calculate_subchannel_friction_factor_const(asm)

    if asm.coolant_int_params['Re'] <= Re_bl:
        return (cfsc['laminar']
                / asm.coolant_int_params['Re_sc']**_m['laminar'])
    elif asm.coolant_int_params['Re'] >= Re_bt:
        return (cfsc['turbulent']
                / asm.coolant_int_params['Re_sc']**_m['turbulent'])
    else:
        f = {}
        for k in ['laminar', 'turbulent']:
            f[k] = cfsc[k] / asm.coolant_int_params['Re_sc']**_m[k]
        # Transition regime intermittency factor
        x = calc_sc_intermittency_factor(asm, Re_bl, Re_bt)
